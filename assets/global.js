...
function update_counter(){
  var target1 = Number($('.mo-progress').attr('target_1')) || window.free_products.target1;
  var target2 = Number($('.mo-progress').attr('target_2')) || window.free_products.target2;
  var target3 = Number($('.mo-progress').attr('target_3')) || window.free_products.target3;
  var product1 = Number($('[product_1]').attr('product_1')) || window.free_products.product1;
  var product2 = Number($('[product_2]').attr('product_2')) || window.free_products.product2;
  $.ajax({
    type: "GET",
    url: "/collections/all",
    dataType: 'html',
    success: function(html){
      var data = $(html).find('.mo-game');
      var data2 = $(html).find('.mo-commands');
      $('.mo-game').replaceWith(data);
      $('.mo-commands').replaceWith(data2);
      if($('[to_remove]').attr('to_remove').includes(',')){
        var rem = $('[to_remove]').attr('to_remove').split(',');
        rem.forEach(function(e){
          var to_remove = Number(e);
          if(to_remove != ''){
              jQuery.post('/cart/update.js', {
                ...
                if($('body').hasClass('template-cart')){
                  var event = new Event("Cart Change");
      ...
      var to_remove = Number($('[to_remove]').attr('to_remove'));
        if(to_remove != ''){
            jQuery.post('/cart/update.js', {
              updates: {
                [to_remove]: 0
        ...
        var to_add = Number($('[to_add]').attr('to_add'));
        
      if($('[to_add]').attr('to_add').includes(',')){
        var add = $('[to_add]').attr('to_add').split(',');
...
update_counter();
document.addEventListener("Cart Change", function(event) { // (1)
    setTimeout(function() {
      location.reload();
    },1500)
  });